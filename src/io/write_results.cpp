...
//
// HighDims note:
//   If baselines::RunReport contains a field `dim`, this TU will automatically
//   include a "dim" column (and JSON field) without requiring callers to change.
//   If not, the schema matches the legacy 2D version.
...
inline std::string SanitizeFilename(std::string_view s) {
  ...
  out.reserve(s.size());
  for (char c : s) {
    // Replace path separators and illegal filename chars with '_'.
    if (c == '/' || c == '\\' || c == ':' || c == '*' || c == '?' || c == '"' ||
        c == '<' || c == '>' || c == '|') {
      out.push_back('_');
    } else if (c == ' ' || c == '\t' || c == '\n' || c == '\r') {
...
inline double GetTopLevelRunTotalMs(const baselines::RunReport& r) {
  // Sum phases that start with "run_" to avoid double counting fine-grained phases.
  double ms = 0.0;
...
struct HasDimMember : std::false_type {};
template <class T>
struct HasDimMember<T, std::void_t<decltype(std::declval<const T&>().dim)>> : std::true_type {};

template <class RR>
inline void AppendOptionalHeaderDim(std::vector<std::string>& header) {
  if constexpr (HasDimMember<RR>::value) {
    header.push_back("dim");
  }
}

template <class RR>
inline void AppendOptionalRowDim(std::vector<std::string>& row, const RR& r) {
  if constexpr (HasDimMember<RR>::value) {
    row.push_back(std::to_string(static_cast<int>(r.dim)));
  }
}

template <class RR>
inline void AppendOptionalJsonDim(std::ostringstream& oss, const RR& r) {
  if constexpr (HasDimMember<RR>::value) {
    oss << "\"dim\":" << static_cast<int>(r.dim) << ",";
  }
}
...
inline std::vector<std::string> RunReportHeader() {
  ...
  AppendOptionalHeaderDim<baselines::RunReport>(h);

  // Rest is stable across 2D/HighDims.
...
inline std::string RunReportToJsonLine(const baselines::RunReport& r) {
  ...
  // Note: phases are embedded as JSON object via PhaseRecorder::ToJsonMillis().
  std::ostringstream oss;
  oss << "{";
...
bool WriteSamplesTSV(const std::string& out_dir,
  ...
  fname << SanitizeFilename(report.dataset_name) << "_"
        ...
        << SanitizeFilename(std::string(ToString(report.variant)));

  if constexpr (HasDimMember<baselines::RunReport>::value) {
    fname << "_d" << static_cast<int>(report.dim);
  }

  fname << "_seed" << report.seed
        << "_t" << report.t
        << ".tsv";
  ...
  std::string io_err;
  if (!csv::WritePairs(path.string(),
                       Span<const PairId>(report.samples.pairs.data(), report.samples.pairs.size()),
                       csv::Dialect{'\t'},
                       &io_err)) {
...
// Caller groups runs (e.g. same dataset/method/variant/dim) before calling.
bool WriteSummaryTSV(const std::string& path,
  ...
  // Optional dim (if present): use the first successful run's dim.
  int dim_val = -1;

  for (usize i = 0; i < runs.size(); ++i) {
    const auto& r = runs[i];
    if (!r.ok) continue;  // summarize successful runs only
    ...
    if constexpr (HasDimMember<baselines::RunReport>::value) {
      if (dim_val < 0) dim_val = static_cast<int>(r.dim);
    }
  }
  ...
  if constexpr (HasDimMember<baselines::RunReport>::value) {
    row.push_back(dim_val >= 0 ? std::to_string(dim_val) : std::string());
  }
