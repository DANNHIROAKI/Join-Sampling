...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--volume_dist", type=str, default="normal", help="Alacarte volume distribution")
    p.add_argument("--volume_cv", type=float, default=0.25, help="Alacarte volume coefficient of variation")
    p.add_argument("--shape_sigma", type=float, default=0.5, help="Alacarte log-shape sigma")
    p.add_argument("--tune_samples", type=int, default=200_000, help="coverage tuning samples")
    p.add_argument("--tune_tol_rel", type=float, default=0.01, help="coverage tuning relative tolerance")
    p.add_argument("--tune_max_iter", type=int, default=30, help="coverage tuning max iterations")
    p.add_argument("--dtype", type=str, default="float32", choices=["float32", "float64"], help="output scalar dtype for generated boxes")
    p.add_argument("--domain_lo", type=float, default=0.0, help="domain lower bound for every axis")
    p.add_argument("--domain_hi", type=float, default=1.0, help="domain upper bound for every axis")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # ----------------------------
    # Configurable parameters (defaults preserve the original fixed setup)
    # ----------------------------
    if not (args.domain_hi > args.domain_lo):
        _eprint("[rectgen][FATAL] domain_hi must be > domain_lo")
        return 2

    universe = None
    if args.domain_lo != 0.0 or args.domain_hi != 1.0:
        lo = np.full(int(args.d), float(args.domain_lo), dtype=np.float64)
        hi = np.full(int(args.d), float(args.domain_hi), dtype=np.float64)
        universe = np.stack([lo, hi], axis=1)

    volume_dist = str(args.volume_dist)
    volume_cv = float(args.volume_cv)
    shape_sigma = float(args.shape_sigma)
    tune_tol_rel = float(args.tune_tol_rel)
    tune_max_iter = int(args.tune_max_iter)
    tune_samples = int(args.tune_samples)
    ...
    report: Dict[str, Any] = {
        ...
        "fixed_params": {
            "universe": None if universe is None else universe.tolist(),
            "volume_dist": volume_dist,
            "volume_cv": float(volume_cv),
            "shape_sigma": float(shape_sigma),
            "tune_samples": int(tune_samples),
            "tune_tol_rel": float(tune_tol_rel),
            "tune_max_iter": int(tune_max_iter),
            "dtype": args.dtype,
            "domain_lo": float(args.domain_lo),
            "domain_hi": float(args.domain_hi),
