...
//  - Side tie-break (R-before-S or S-before-R)
//
...
static void TestOrderingRBeforeS(TestContext& t) {
  ...
  // At x=0: START events exist for r0, r1, s0.
  // With RBeforeS, all R STARTs come before S STARTs. Within same side, by id.
  ...
  CHECK_EQ(t, events[1].side, Side::R);
  CHECK_EQ(t, events[1].id, 1u);

  CHECK_EQ(t, events[2].x, 0.0);
  CHECK_EQ(t, events[2].kind, EventKind::Start);
  CHECK_EQ(t, events[2].side, Side::S);
  ...
  // First at x=1 should be R END of r1.
  CHECK_EQ(t, events[b].kind, EventKind::End);
  CHECK_EQ(t, events[b].side, Side::R);
  CHECK_EQ(t, events[b].id, 1u);

  // Second at x=1 should be S END of s0.
  CHECK_EQ(t, events[b + 1].x, 1.0);
  CHECK_EQ(t, events[b + 1].kind, EventKind::End);
  CHECK_EQ(t, events[b + 1].side, Side::S);
...
static void TestOrderingSBeforeR(TestContext& t) {
  ...
  // At x=0, S START should come before R START when SBeforeR.
  CHECK_EQ(t, events[0].x, 0.0);
  CHECK_EQ(t, events[0].kind, EventKind::Start);
