...
//  - Optionally read simple box CSV (debug / small-medium datasets)
//
// Writer supports proper escaping for separators/quotes/newlines.
// Reader is intentionally simple and does NOT support quoted separators.
//
// Expected box CSV format (header optional):
//   id, lo0, lo1, ..., lo(d-1), hi0, hi1, ..., hi(d-1)
// If id column is missing, sequential ids are used.
//
// Half-open semantics are not “enforced” here beyond parsing numbers; validation is
// handled by Relation/Dataset Validate() and/or caller options.
...
bool WriteBoxes(const std::string& path,
  ...
  for (usize i = 0; i < rel.boxes.size(); ++i) {
    ...
    for (int k = 0; k < Dim; ++k) row.push_back(std::to_string(rel.boxes[i].lo.v[static_cast<usize>(k)]));
    for (int k = 0; k < Dim; ++k) row.push_back(std::to_string(rel.boxes[i].hi.v[static_cast<usize>(k)]));
