...
//
// Notes:
//  - Scalars can be stored as float32 or float64; we write float64 by default.
...
bool WriteRelationBinary(const std::string& path,
  ...
  if (opt.scalar == ScalarEncoding::Float64) {
    for (usize i = 0; i < rel.boxes.size(); ++i) {
      ...
      for (int d = 0; d < Dim; ++d) {
        const double x = static_cast<double>(b.lo.v[static_cast<usize>(d)]);
        if (!WriteScalar(out, x, err)) return false;
      }
      for (int d = 0; d < Dim; ++d) {
        const double x = static_cast<double>(b.hi.v[static_cast<usize>(d)]);
        if (!WriteScalar(out, x, err)) return false;
      }
      if (has_ids) {
    ...
    for (usize i = 0; i < rel.boxes.size(); ++i) {
      const auto& b = rel.boxes[i];
      for (int d = 0; d < Dim; ++d) {
        const float x = static_cast<float>(b.lo.v[static_cast<usize>(d)]);
        if (!WriteScalar(out, x, err)) return false;
      }
      for (int d = 0; d < Dim; ++d) {
        const float x = static_cast<float>(b.hi.v[static_cast<usize>(d)]);
        if (!WriteScalar(out, x, err)) return false;
      }
      if (has_ids) {
        const Id id = rel.ids.empty() ? tmp_ids[i] : rel.ids[i];
