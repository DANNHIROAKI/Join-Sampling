...
// sjs/io/dataset.h
//
// Dataset container types for Spatial Join Sampling experiments (HighDims).
//
// Design goals:
//  - Minimal dependencies (core + geometry only).
//  - Support high-dimensional input (templated by Dim).
//  - Keep object IDs stable across reordering (optional ids vector).
//  - Provide common metadata (name, domain bounds) for experiment logging.
//
// Notes:
...
struct Dataset {
  ...
  std::string name;   // dataset name / tag
  bool half_open = true;  // project convention: half-open boxes
  ...
  // Domain bounds for embedding-based baselines (KD/RangeTree).
