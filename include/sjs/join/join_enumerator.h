...
struct PlaneSweepOptions {
  ...
  // For HighDims, axis defaults to 0 (dimension 1 in the paper).
  int axis = 0;

  // Deterministic tie-break between R/S when x and kind tie (only if ids tie).
...
//
// Stream order:
//  - Event order: BuildSweepEvents + EventLess (axis + half-open + id total order).
