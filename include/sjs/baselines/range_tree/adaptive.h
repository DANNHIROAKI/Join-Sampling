...
//
// RangeTree baseline (HighDims) — Adaptive variant (Framework III).
//
// Implements the budgeted caching Framework III from SJS-HighDims.md, using
// the Chapter-4 RangeTree event-block primitive (dynamic 2(d-1)-dim range tree
// used under an axis-0 sweep).
//
// Pass 1 (sweep):
...
//
// Correctness: distribution matches Framework II; caching only affects performance.
...
inline double SlotScorePoisson(u64 w_i,
                               ...
                               u64 t,
                               u32 r) noexcept {
  if (w_i == 0 || t == 0 || r == 0) return -std::numeric_limits<double>::infinity();

  // Expected number of times this event is selected by the t slots.
  const long double W_hat = EstimateTotalW(total_events, i, w_sofar);
  if (!(W_hat > 0.0L)) return -std::numeric_limits<double>::infinity();

  const long double p_hat = static_cast<long double>(w_i) / W_hat;
  const double mu = static_cast<double>(static_cast<long double>(t) * p_hat);

  // Value ~ P[Poisson(mu) >= r]
  return PoissonSurvival(mu, r);
}

// Upper bound on r such that P(Poisson(mu) >= r) is not astronomically tiny.
inline u32 UpperBoundPoissonTail(double mu, double tol = 1e-12) noexcept {
  if (!(mu > 0.0) || !std::isfinite(mu)) return 0U;
  if (mu < 1e-6) return 1U;

  // Conservative bound: r <= mu + 10*sqrt(mu) + 10
  const double r0 = mu + 10.0 * std::sqrt(mu) + 10.0;
  u32 r = static_cast<u32>(std::ceil(r0));
  if (r < 1U) r = 1U;

  // Tighten by decrement until survival >= tol.
  while (r > 1U) {
    if (PoissonSurvival(mu, r) >= tol) break;
    --r;
  }
  return r;
}
...
// --------------------------
// Prefetch heap of valued slots (min-heap by score).
// --------------------------
struct PrefetchSlot {
  double score;
  u32 sid;
...
struct PrefetchSlotGreater {
  bool operator()(const PrefetchSlot& a, const PrefetchSlot& b) const noexcept {
    return a.score > b.score;  // for min-heap via std::priority_queue
  }
};

class PrefetchHeap {
 public:
  void Clear() { while (!pq_.empty()) pq_.pop(); }

  usize Size() const noexcept { return pq_.size(); }

  double MinScore() const noexcept {
    if (pq_.empty()) return -std::numeric_limits<double>::infinity();
    return pq_.top().score;
  }

  void Push(const PrefetchSlot& s) { pq_.push(s); }

  PrefetchSlot PopMin() {
    SJS_DASSERT(!pq_.empty());
    PrefetchSlot m = pq_.top();
    pq_.pop();
    return m;
  }

 private:
  std::priority_queue<PrefetchSlot, std::vector<PrefetchSlot>, PrefetchSlotGreater> pq_;
};
...
class RangeTreeAdaptiveBaseline final : public IBaseline<Dim, T> {
 public:
  static_assert(Dim >= 2, "RangeTreeAdaptiveBaseline requires Dim>=2");

  using DatasetT = Dataset<Dim, T>;
  using BoxT = Box<Dim, T>;
  using Ctx = detail::RangeTreeContext<Dim, T>;
  using IndexT = detail::RangeTreeIndex<Dim, T>;
  ...
  bool Count(const Config& cfg, Rng* rng, CountResult* out, PhaseRecorder* phases, std::string* err) override {
    ...
    for (usize pos = 0; pos < events.size(); ++pos) {
      ...
      if (sid_u >= E) {
        if (err) *err = "RangeTreeAdaptiveBaseline::Count: sid out of range";
        ctx_.ResetActive();
        return false;
      }
      ...
      if (can_full_cache) {
        ...
        if (len != w) {
          if (err) *err = "RangeTreeAdaptiveBaseline::Count: full cache REPORT size mismatch (bug)";
          ctx_.ResetActive();
          return false;
        }
        ...
        if (B_samp > 0) {
          ...
          while (true) {
            ...
            const double score = detail::SlotScorePoisson(w, static_cast<u32>(E), i, W_sofar, cfg.run.t, r);
          ...
          if (s_keep > 0) {
            ...
            std::vector<u32> samp;
            samp.clear();
            other.Sample(q, s_keep, &rng_samp, &samp);
            if (samp.size() != static_cast<usize>(s_keep)) {
              if (err) *err = "RangeTreeAdaptiveBaseline::Count: prefetch SAMPLE failed";
              ctx_.ResetActive();
              return false;
            }
  ...
  bool Sample(const Config& cfg, Rng* rng, SampleSet* out, PhaseRecorder* phases, std::string* err) override {
    ...
    if (cfg.run.t > static_cast<u64>(std::numeric_limits<u32>::max())) {
      if (err) *err = "RangeTreeAdaptiveBaseline::Sample: run.t too large (must fit in u32)";
      return false;
    }
    ...
    if (!alias.BuildFromU64(w_total_)) {
      // All weights zero? (should have W_==0) but keep safe.
      return true;
    }

    // ---------------------------------------------------------------------
    // Randomness protocol (SJS-HighDims §3.1.4): split the provided RNG into
    // independent sub-streams.
    //
    // IMPORTANT: do NOT use cfg.run.seed for per-event seeding here.
    // The experiment runner applies per-repeat offsets (cfg.run.seed + rep)
    // and passes phase-separated RNG streams. Using cfg.run.seed would ignore
    // that offset, causing repeats to share within-event randomness.
    // ---------------------------------------------------------------------
    ...
    std::vector<SlotAssign> assigns;
    assigns.resize(static_cast<usize>(t));
    for (u32 slot = 0; slot < t; ++slot) {
      const u32 sid = static_cast<u32>(alias.Sample(&rng_assign));
      assigns[static_cast<usize>(slot)] = SlotAssign{sid, slot};
    }
    std::sort(assigns.begin(), assigns.end(), by_sid_slot);

    // Build per-event ranges in assigns.
    std::vector<u32> off;
    off.assign(E + 1, 0U);
    {
      u32 cur = 0;
      for (u32 sid = 0; sid < static_cast<u32>(E); ++sid) {
        off[static_cast<usize>(sid)] = cur;
        while (cur < t && assigns[static_cast<usize>(cur)].sid == sid) ++cur;
      }
      off[static_cast<usize>(E)] = t;
    }

    out->pairs.resize(static_cast<usize>(t));

    // Residual slots: for each event sid, store which output slots still need sampling.
    std::vector<std::vector<u32>> residual;
    residual.assign(E, std::vector<u32>{});

    // -----------------
    // Fill from full cache / prefetched samples; record residual.
    // -----------------
    const DatasetT* ds = ctx_.dataset();
    SJS_DASSERT(ds != nullptr);

    auto emit_pair = [&](u32 sid, u32 partner_handle, u32 slot_idx) {
      const join::Side side = start_side_[static_cast<usize>(sid)];
      const u32 q_handle = start_index_[static_cast<usize>(sid)];

      PairId p{};
      if (side == join::Side::R) {
        p.r = ds->R.GetId(static_cast<usize>(q_handle));
        p.s = ds->S.GetId(static_cast<usize>(partner_handle));
      } else {
        p.r = ds->R.GetId(static_cast<usize>(partner_handle));
        p.s = ds->S.GetId(static_cast<usize>(q_handle));
      }
      out->pairs[static_cast<usize>(slot_idx)] = p;
    };

    for (u32 sid = 0; sid < static_cast<u32>(E); ++sid) {
      const u32 begin = off[static_cast<usize>(sid)];
      const u32 end = off[static_cast<usize>(sid + 1U)];
      if (begin == end) continue;

      const usize sid_u = static_cast<usize>(sid);

      if (cached_[sid_u]) {
        // Fully cached: sample from cached partners.
        const u64 offp = cache_off_[sid_u];
        const u64 lenp = cache_len_[sid_u];
        if (lenp == 0) {
          // Should not happen when cached_[sid_u]==1.
          for (u32 j = begin; j < end; ++j) {
            residual[sid_u].push_back(assigns[static_cast<usize>(j)].slot);
          }
          continue;
        }

        // Use a per-event RNG stream so cache sampling is independent from
        // slot assignment randomness.
        Rng crng(DeriveSeed(seed_cache, 0xCA11CEULL, static_cast<u64>(sid)));
        for (u32 j = begin; j < end; ++j) {
          const u64 pick = crng.UniformU64(lenp);
          const u32 partner = cache_partners_[static_cast<usize>(offp + pick)];
          emit_pair(sid, partner, assigns[static_cast<usize>(j)].slot);
        }
        continue;
      }
      ...
      const u32 have = static_cast<u32>(pref.size());
      const u32 need = end - begin;
      const u32 use = (have < need) ? have : need;

      for (u32 j = 0; j < use; ++j) {
        emit_pair(sid, pref[static_cast<usize>(j)], assigns[static_cast<usize>(begin + j)].slot);
      }

      for (u32 j = begin + use; j < end; ++j) {
        residual[sid_u].push_back(assigns[static_cast<usize>(j)].slot);
      }
    }

    // Count residual total.
    u64 residual_total = 0;
    for (usize i = 0; i < E; ++i) residual_total += static_cast<u64>(residual[i].size());

    if (residual_total == 0) {
      // All slots filled without a second pass.
      return true;
    }

    // -----------------
    // Pass 2: second sweep to sample residual slots only.
    // -----------------
    auto scoped = phases ? phases->Scoped("phase2_residual_sample") : PhaseRecorder::ScopedPhase(nullptr, "");
    ...
    for (usize pos = 0; pos < events.size(); ++pos) {
      ...
      const usize sid_u = static_cast<usize>(sid);
      if (sid_u >= E) {
        if (err) *err = "RangeTreeAdaptiveBaseline::Sample: sid out of range in pass2";
        ctx_.ResetActive();
        return false;
      }

      if (!residual[sid_u].empty()) {
        // Need to sample k partners for this event.
        const bool q_is_r = (ev.side == join::Side::R);
        const BoxT& q = ctx_.BoxOf(ev);
        IndexT& other = q_is_r ? ctx_.active_s() : ctx_.active_r();

        const u32 k = static_cast<u32>(residual[sid_u].size());
        Rng rng_e(DeriveSeed(seed_sweep, 0xBADC0FFEULL, static_cast<u64>(sid)));

        std::vector<u32> samp;
        samp.clear();
        other.Sample(q, k, &rng_e, &samp);
        if (samp.size() != static_cast<usize>(k)) {
          if (err) *err = "RangeTreeAdaptiveBaseline::Sample: pass2 SAMPLE failed";
          ctx_.ResetActive();
          return false;
        }

        for (usize j = 0; j < samp.size(); ++j) {
          const u32 slot_idx = residual[sid_u][j];
          emit_pair(sid, samp[j], slot_idx);
        }
        residual[sid_u].clear();
      }
 ...
 private:
  ...
  // weights per START-event
  std::vector<u64> w_total_;

  // start-id -> (side, handle)
