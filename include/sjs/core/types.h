...
//  - Stable, explicit integer aliases.
//  - Generic Dim helpers (works for high-dimensional upgrades).
//  - Lightweight utilities (Span, parsing helpers) used across modules.
...
// Soft cap used for sanity checks and template guards.
// (Dispatch dims is controlled elsewhere via SJS_DIMS in dim_dispatch.h.)
inline constexpr int kMaxSupportedDim = 64;
...
struct PairIdHash {
  usize operator()(const PairId& p) const noexcept {
    // Pack two 32-bit ids into 64-bit, then hash.
    const u64 key = (static_cast<u64>(p.r) << 32) ^ static_cast<u64>(p.s);

    // splitmix64 finalizer for better diffusion.
...
// ASCII-lowercase helper (locale-free).
...
inline bool ParseVariant(std::string_view s, Variant* out) noexcept {
  ...
  if (detail::EqualsIgnoreCase(s, "enum_sampling") ||
      detail::EqualsIgnoreCase(s, "enumerate_sampling") ||
      detail::EqualsIgnoreCase(s, "enumerate+sampling") ||
      detail::EqualsIgnoreCase(s, "enum+sampling") ||
      detail::EqualsIgnoreCase(s, "enum")) {
...
inline bool ParseMethod(std::string_view s, Method* out) noexcept {
  ...
  if (detail::EqualsIgnoreCase(s, "ours")) {
    *out = Method::Ours;
    return true;
  }
  if (detail::EqualsIgnoreCase(s, "kd_tree") ||
      detail::EqualsIgnoreCase(s, "kdtree") ||
      detail::EqualsIgnoreCase(s, "kd")) {
    *out = Method::KDTree;
    return true;
  }
  if (detail::EqualsIgnoreCase(s, "range_tree") ||
      detail::EqualsIgnoreCase(s, "rangetree") ||
      detail::EqualsIgnoreCase(s, "rtree")) {
    *out = Method::RangeTree;
    return true;
  }
...
class Span {
 public:
  ...
  constexpr Span subspan(usize offset, usize count) const noexcept {
    ...
    const usize remain = size_ - offset;
    const usize n = (count <= remain) ? count : remain;
