...
//
// Heterogeneous box sizes: mixture of small and large boxes.
//
...
//      If true, for a large box pick ONE axis to be large and keep other axes small.
...
class HeteroSizesGenerator final : public ISyntheticGenerator<Dim, T> {
 public:
  ...
  bool Generate(const DatasetSpec& spec, DatasetT* out_ds, Report* report, std::string* err) override {
    ...
    if (!detail::CheckIdFits(spec.n_r, err, "HeteroSizesGenerator(R)")) return false;
    if (!detail::CheckIdFits(spec.n_s, err, "HeteroSizesGenerator(S)")) return false;
      ...
      for (u64 i = 0; i < n; ++i) {
        ...
        for (int axis = 0; axis < Dim; ++axis) {
          ...
          if (!is_large) {
            ...
            if (!anisotropic || axis == large_axis) w_frac = rng.UniformDouble(wl_min, wl_max);
            else w_frac = rng.UniformDouble(ws_min, ws_max);
          }
    ...
    if (shuffle_r) {
      Rng rng_shuf(DeriveSeed(spec.seed, 0x55555555ULL));
      detail::ShuffleRelation(&R, &rng_shuf);
    }
    if (shuffle_s) {
      Rng rng_shuf(DeriveSeed(spec.seed, 0x66666666ULL));
