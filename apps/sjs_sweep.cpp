...
sjs::apps::ResolveDatasetPathsFromSweep(fs::path(sweep_path), &spec.base);
  ...
  if (!sjs::IsSupportedExperimentDim(spec.base.dataset.dim)) {
    SJS_LOG_ERROR(sjs::SupportedDimError(spec.base.dataset.dim));
    return 4;
  }

  return sjs::DispatchSupportedDim(spec.base.dataset.dim, [&](auto tag) {
  ...
  if (spec.base.dataset.source != sjs::DataSource::Synthetic) {
    std::string local_err;
    if (!sjs::apps::LoadOrGenerateDataset<Dim>(spec.base, &fixed_ds, &fixed_gen_report, &local_err)) {
      ...
      for (double alpha : alpha_list) {
        for (u64 gen_seed : gen_seeds) {
          ...
          if (sweep_dataset) {
            ...
            std::string local_err;
            if (!sjs::apps::LoadOrGenerateDataset<Dim>(cfg_ds, &ds, &gen_report, &local_err)) {
          ...
          for (u64 t : spec.t_list) {
            for (sjs::Method method : spec.methods) {
              for (sjs::Variant variant : spec.variants) {
                ...
                // Create baseline.
                std::string b_err;
                auto baseline = sjs::baselines::CreateBaseline<Dim>(method, variant, &b_err);
                if (!baseline) {
