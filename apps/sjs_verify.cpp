...
inline void PrintUsage() {
  std::cerr
      ...
      << "Baselines supported in this build (Dim=2/3/4/5):\n"
      << sjs::baselines::BaselineHelpSupportedDims()
      << "\n";
}
...
template <int Dim>
int RunMainForDim(const sjs::Config& cfg, const sjs::ArgMap& args) {
  using sjs::u64;
  using sjs::usize;
  ...
  ParseU64(args.Get("oracle_max_checks"), &oracle_max_checks);
  ParseU64(args.Get("oracle_collect_limit"), &oracle_collect_limit);
  ParseU64(args.Get("oracle_cap"), &oracle_cap);

  sjs::Dataset<Dim, sjs::Scalar> ds;
  sjs::synthetic::Report gen_report;
  if (!LoadOrGenerateDataset<Dim>(cfg, &ds, &gen_report, &err)) {
  ...
  const u64 oracle_count = sjs::join::CountNaive<Dim, sjs::Scalar>(ds.R, ds.S, &oracle_stats);
  ...
  auto baseline = sjs::baselines::CreateBaseline<Dim>(cfg, &err);
  if (!baseline) {
  ...
  for (u64 rep = 0; rep < repeats; ++rep) {
    ...
    const double rel = RelErr(est, truth);
